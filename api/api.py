...
c = config = {
    ...
    'target_dir': None,
    'movie_filename': None #FIXME: allow multiple filename patterns?
}
...
def get_files(filter='.*(avi|mpg|jpg)$'):
    """ Returns all files contained in the motion folder (target_dir) """
    """ filtered by the optional <filter> consisting in a regexp """
...
@app.route('/events', name='api-events')
def get_events():
    """ Return an list of motion events generated by parsing motion files """
...
def get_meta(file):
    """ Returns video metadata for the given <file> """
...
def get_preview(file, time=0, size=None):
    """ Returns a frame from the given <file> """
    """ extracting frame at <time> and generating an jpeg of <size> """
    file = '/' + file
    if (not os.path.isfile(file)): abort(404, 'File %s does not exist' % file)
...
def setup():
    def motion_conf():
        """ Parses motion.conf and returns a dict """
        """ containing a subset of directives """
        match = re.compile(
